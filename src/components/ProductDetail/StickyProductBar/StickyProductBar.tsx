...
interface StickyProductBarProps {
    isVisible: boolean;
    // Thêm các props dữ liệu
    image?: string;
    name?: string;
    price?: number;
    originalPrice?: number;
    sku?: string;
    // Optional: Callbacks cho nút bấm
    onAddToCart?: () => void;
    onBuyNow?: () => void;
}
            ...
            <div className="container spb-container">
                ...
                <div className="spb-left">
                    <div className="spb-img-box">
                        <img src={image || "https://placehold.co/50x50?text=No+Img"} alt={name} />
                    </div>
                    <div className="spb-info">
                        <div className="spb-title-row">
                            <h3 className="spb-name">{name || "Đang tải..."}</h3>
                            {sku && <span className="spb-sku">({sku})</span>}
                        </div>
                        {/* Có thể thêm prop variantName nếu muốn hiển thị chi tiết "256GB, Cam" */}
                    </div>
                </div>
                ...
                <div className="spb-right">
                    <div className="spb-price-group">
                        <p className="spb-current-price">{formatCurrency(price)}</p>
                        
                        {originalPrice > price && (
                            <div className="spb-old-price-row">
                                <span className="spb-old-price">{formatCurrency(originalPrice)}</span>
                                <span className="spb-discount">-{discountPercent}%</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="spb-actions">
                        {/* Nút Giỏ hàng */}
                        <button className="spb-btn-cart icon-btn" onClick={onAddToCart}>
                        ...
                        <button className="spb-btn-buy" onClick={onBuyNow}>
                            Mua ngay
                        </button>
                        
                        <button className="spb-btn-installment">
                            <span className="ins-label">Trả góp</span>
                            <span className="ins-sub">0% lãi suất</span>
                        </button>
                    </div>
                </div>
            </div>
